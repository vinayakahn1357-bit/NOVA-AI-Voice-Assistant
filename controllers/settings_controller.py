...
Handles GET/POST settings and model listing.
"""
...
def get_current_settings() -> dict:
    """Return the current settings with masked API keys."""
...
def update_settings(data: dict) -> dict:
    """
    Update settings from a request payload.
    ...
    log.info("Settings updated: provider=%s, model=%s", settings["provider"], settings["model"])
...
def list_models() -> dict:
    ...
    if provider == "ollama_cloud":
        try:
            ...
            cloud_headers = {"Authorization": f"Bearer {settings['ollama_api_key']}"}
